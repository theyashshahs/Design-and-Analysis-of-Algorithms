#include<bits/stdc++.h>

using namespace std;

void merge(vector<int>& arr, long int l, long int m, long int r) {
    int i, j, k;
    int n1 = m - l + 1;
    int n2 = r - m;

    int L[n1], R[n2];

    for(i=0; i < n1; i++)
        L[i] = arr[l+i];
    
    for(j=0; j < n2; j++)
        R[j] = arr[m+1+j];

    i = 0;
    j = 0;
    k = l;

    while(i< n1 && j< n2)
    {
        if(L[i] <= R[j]) 
        {
            arr[k] = L[i];
            i++;
        }

        else
        {
            arr[k] = R[j];
            j++;
        }
        k++;
    }

    while (i < n1)
    {
        arr[k] = L[i];
        i++;
        k++;
    }

    while (j < n2)
    {
        arr[k] = R[j];
        j++;
        k++;
    }
}

void mergesort(vector<int>& arr, long int l, long int r) {
    if(l < r)
    {
        int m = (l + (r-1))/2;

        mergesort(arr, l, m);
        mergesort(arr, m+1, r);

        merge(arr, l, m, r);
    }
}


int main() {

    // cin.tie(NULL);
    // cout.tie(NULL);
    // ios::sync_with_stdio(0);

    ifstream fin;
    ofstream fout;
    int ch;
    vector<int> v,v1;
    clock_t end1, start1;

    
    fin.open("File 6.txt", ios::in);    //change filename

    while (!fin.eof())
    {
        fin >> ch;
        v.push_back(ch);
    }
    fin.close();
   
    //Average Case file execution
    start1 = clock();
    mergesort(v, 0, v.size() - 1);
    end1 = clock();

    fout.open("File 6sort.txt", ios::out);  //change filename 
    for(int i = 0; i < v.size(); i++ )
    fout << v[i] << "\n";
    fout.close();

    
    //Best Case file execution
    v1 = v;
    sort(v1.begin(), v1.end());

    clock_t start2 = clock();
    mergesort(v1, 0, v1.size() - 1);
    clock_t end2 = clock();

    //Reverse sorted file
    reverse(v1.begin(), v1.end());

    fout.open("File 6sortrev.txt", ios::out);  //change filename
    for(int i = 0; i < v1.size(); i++ )
    fout << v1[i] << "\n";

    fout.close();

    clock_t start3 = clock();
    mergesort(v1, 0, v1.size() - 1);
    clock_t end3 = clock();

    
    cout<<"\nData for 6th File :- \n";
    cout<<std::fixed << "\nExecutable time for best case file : "<< (double)(end2 - start2)/CLOCKS_PER_SEC << " seconds " << endl;
    cout<<std::fixed << "\nExecutable time for average case file : "<< (double)(end1 - start1)/CLOCKS_PER_SEC << " seconds " << endl;
    cout<<std::fixed << "\nExecutable time for worst case file : "<< (double)(end3 - start3)/CLOCKS_PER_SEC << " seconds " << endl;
    return 0;
}